...
function App() {
  const [random, setRandom] = useState(
    Math.floor(Math.random() * soundsFormated.length)
  );
  ...
  const showFavs = () => {
    if (!isFavsOnly) {
      const favsAudios = soundsFormated.filter((a, index) =>
        favs?.includes(a.id)
      );
      setAudios(
        favsAudios.map((sound) => {
          return {
            audio: new Audio(`/DAVAsound/${sound.name}`),
            isPlaying: false,
            id: sound.id,
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="first-buttons-container">
          <button
            className={isFavsOnly ? "btn-fav" : "btn"}
            onClick={() => showFavs()}>
            Favs
          </button>
          ...
          <Holdable
            ...
            onClick={
              isFavsOnly
                ? () => console.log("C'est vraiment un site de merde")
                : () => start(rd)
            }
            id={rd}>
            <button className="btn">Clear favs</button>
          </Holdable>
        </div>
